...
`XDRServer` (port 7373) is the control surface. `XdrFacade` brokers between the server and `TunerController` + `ScanEngine`. `ScanEngine` uses `SignalLevel` (the dBFS / noise-floor / SNR / clip estimator — see `[SIG]` log lines; its SNR term also caps the displayed meter, see `snrLevel120FromSnrDb`) to drive scan decisions. `XDRServer` speaks the XDR-GTK / FM-DX protocol and authenticates with OpenSSL (constant-time hash compare; fails closed if the secure RNG is unavailable).
